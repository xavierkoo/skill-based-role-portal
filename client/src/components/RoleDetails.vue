...
const visibleSkills = ref(roleDetails.skills.slice(0, maxSkillsToShow))
const remainingSkills = ref(roleDetails.skills.slice(maxSkillsToShow))
const showMore = ref(remainingSkills.value.length > 0)

const toggleShowMore = () => {
  if (showMore.value) {
    visibleSkills.value = roleDetails.skills
...
<template>
  <div class="roleDetails container px-5">
    <h1 class="check">{{ roleDetails.roleName ? roleDetails.roleName : 'TBC' }}</h1>
    <div class="details">
      <div>
        <span class="fw-bold">Posted On:</span>
        <span class="check">{{ roleDetails.postedOn ? roleDetails.postedOn : 'TBC' }}</span>
      </div>
      <div>
        <span class="fw-bold">Posted By:</span>
        <a class="f-underline check">{{
          roleDetails.creator.name ? roleDetails.creator.name : 'TBC'
        }}</a>
      </div>
    </div>
    <div>
      <span class="fw-bold">Deadline:</span>
      <span class="check">{{ roleDetails.deadLine ? roleDetails.deadLine : 'TBC' }}</span>
    </div>
    <br />
    ...
    <div class="description">
      {{ roleDetails.description ? roleDetails.description : 'No description available' }}
    </div>
  </div>
</template>
